...
function buildSheet(rows, colWidths){
  ...
  xml += '</cols><sheetData>';
  const allMerges = rows.flatMap(r=>r.merges||[]);
  rows.forEach(r=>{
  ...
  xml+='</sheetData>';
  if(allMerges.length){
    const unique=[...new Set(allMerges)];
    xml+=`<mergeCells count="${unique.length}">${unique.map(m=>`<mergeCell ref="${m}"/>`).join('')}</mergeCells>`;
  }
  xml+='</worksheet>';
  return xml;
}
...
function zipFiles(files){
  ...
  for(const[name,content]of Object.entries(files)){
    ...
    const dataBuf=typeof content==='string'?enc.encode(content):content;
    const crc=crc32(dataBuf);
    const lh=new Uint8Array([0x50,0x4B,0x03,0x04,20,0,0,0,0,0,0,0,0,0,...u32(crc),...u32(dataBuf.length),...u32(dataBuf.length),...u16(nameBuf.length),...u16(0),...nameBuf]);
    centralDir.push({name:nameBuf,crc,size:dataBuf.length,offset});
    entries.push(lh,dataBuf);
    offset+=lh.length+dataBuf.length;
  }
  const cdOffset=offset;
  const cdEntries=centralDir.map(({name,crc,size,offset})=>new Uint8Array([0x50,0x4B,0x01,0x02,20,0,20,0,0,0,0,0,0,0,0,0,...u32(crc),...u32(size),...u32(size),...u16(name.length),...u16(0),...u16(0),0,0,0,0,0,0,0,0,...u32(offset),...name]));
  ...
  const all=[...entries,...cdEntries,eocd];
...
function buildXlsx(klient,zakazka,adresa,datum,technik,items){
  ...
  blankRow(8,S_INFO_LBL);
  blankRow(6,S_INFO_LBL);
  ...
  blankRow(10,S_INFO_LBL);
  blankRow(6,S_DEF);

  const headers=['Místnost','Šířka (mm)','Výška (mm)','m²','Barva profilu','Kód látky','Vodící lišta','Typ uchycení','Tyč (mm)','Ks','Poznámka'];
  ...
  items.forEach((o,i)=>{
    ...
    rows.push({ri:ri++,ht:20,cells:[
      {v:o.mis,s},{v:o.s,s},{v:o.v,s},{v:m2v,s},
      {v:o.pro,s},{v:o.lat,s},{v:o.lis,s},{v:o.uch,s},
      {v:o.tyc||'',s},{v:o.p,s},{v:o.poz,s}
    ],merges});
  });
...
document.getElementById('btn-add').addEventListener('click',function(){
  ...
  ['f-mis','f-s','f-v','f-lat','f-pro','f-lis','f-tyc','f-uch','f-poz'].forEach(function(id){document.getElementById(id).value='';});
