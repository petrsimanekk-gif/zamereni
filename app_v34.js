...
document.addEventListener('DOMContentLoaded',async function(){
  if(await jeProhlasen()){zobrazApp();}else{zobrazPrihlaseni();}

  document.getElementById('btn-login').addEventListener('click',async function(){
    const zadane=document.getElementById('login-heslo').value;
    const err=document.getElementById('login-err');
    if(zadane===HESLO){
      const h=await hashHesla(HESLO);
      localStorage.setItem(KLIC,h);
      zobrazApp();
    } else {
      err.style.display='block';
      document.getElementById('login-heslo').value='';
      document.getElementById('login-heslo').focus();
    }
  });

  document.getElementById('login-heslo').addEventListener('keydown',function(e){
...
const PRODUCTS={
  ...
  sit_posuvram:{label:'Posuvná síť — v rámu',hasM2:true,
    ...
    getM2:o=>m2(o.sirka,o.vyska)},
  sit_posuvlist:{label:'Posuvná síť — v lištách',hasM2:true,
    headers:['Pozice','Typ profilu','Počet ks','Šířka křídla (mm)','Výška vč. vod. lišt (mm)','m²','Délka vod. lišt (mm)','Poloha příčky','Barva profilu','Barva síťoviny','Poznámka'],
    colWidths:[14,14,8,16,20,8,18,18,14,12,24],
    ...
    toRow:o=>[o.mis,o.typ_profil,+o.pocet||1,+o.sirka_kridla,+o.vyska_vod,m2(o.sirka_kridla,o.vyska_vod),+o.delka_list||'',o.poloha_pricka+(o.pricka_mm?' ('+o.pricka_mm+' mm)':''),o.bprofilu,o.bsitoviny,o.poznamka],
    getM2:o=>m2(o.sirka_kridla,o.vyska_vod)},
...
function saveState(){try{localStorage.setItem(STORE,JSON.stringify({klient:g('klient'),zakazka:g('zakazka'),adresa:g('adresa'),datum:g('datum'),technik:g('technik'),techniktel:g('technik-tel'),items,activTyp}));}catch(e){}}
function loadState(){try{const raw=localStorage.getItem(STORE);if(!raw)return;const s=JSON.parse(raw);['klient','zakazka','adresa','datum','technik'].forEach(id=>{if(s[id])document.getElementById(id).value=s[id];});if(s.techniktel){const el=document.getElementById('technik-tel');if(el)el.value=s.techniktel;}if(s.items)items=s.items;if(s.activTyp)activTyp=s.activTyp;}catch(e){}}
window.resetState=function(){if(!confirm('Smazat zakázku a začít novou?\n(Jméno a telefon technika zůstanou)'))return;try{localStorage.removeItem(STORE);}catch(e){}items=[];activTyp=null;['klient','zakazka','adresa'].forEach(id=>document.getElementById(id).value='');document.getElementById('datum').value=new Date().toISOString().split('T')[0];var sel=document.getElementById('typ-select');if(sel)sel.value='';document.getElementById('form-box').style.display='none';saveState();renderList();};
...
function renderList(){
  ...
  var btnObj=document.getElementById('btn-obj');if(btnObj)btnObj.disabled=items.length===0;
  ...
  list.innerHTML=items.map((o,i)=>{
    ...
    return '<div class="card'+(isEditing?' card-editing':'')+'">'
      ...
      +'<button class="edit-btn" data-copy="'+i+'" title="Kopírovat">⧉</button>'
...
function copyItem(i){
  const o=JSON.parse(JSON.stringify(items[i]));
  items.splice(i+1,0,o);
  saveState();renderList();
  startEdit(i+1);
  var addBtn=document.getElementById('btn-pridat');
  addBtn.textContent='✔ uložit kopii';
  addBtn.style.cssText='background:#cb1419;color:#fff;border:none;width:100%;padding:10px;border-radius:8px;font-size:13px;cursor:pointer;margin-top:4px';
}
...
document.getElementById('list').addEventListener('click',function(e){
  ...
  if(copyBtn){var idx=parseInt(copyBtn.getAttribute('data-copy'));if(!isNaN(idx))copyItem(idx);return;}
...
document.getElementById('btn-pdf').addEventListener('click',function(){
  const klient=g('klient')||'—',zakazka=g('zakazka')||'—',adresa=g('adresa')||'—';
  const datum=formatDatum(g('datum')),technik=g('technik')||'—',techniktel=g('technik-tel')||'';
...
body{font-family:Arial,sans-serif;color:#1a1a1a;background:#fff;padding:0}.page{max-width:800px;margin:0 auto;padding:40px}
...
.footer-left{font-size:13px;color:#1a1a1a;line-height:1.7}
.footer-right{text-align:right;font-size:13px;color:#1a1a1a;line-height:1.7}
.footer-brand{font-size:14px;font-weight:700;color:#cb1419;margin-bottom:2px}
...
.print-bar{background:#fff;border-bottom:2px solid #cb1419;padding:12px 24px;display:flex;align-items:center;gap:16px}
.print-btn{padding:11px 24px;background:linear-gradient(135deg,#8a0c10,#cb1419);color:#fff;border:none;border-radius:8px;font-size:14px;font-weight:700;cursor:pointer;font-family:Arial,sans-serif}
.print-btn:hover{opacity:.9}
.print-hint{font-size:12px;color:#999}
</style></head><body>
<div class="print-bar">
  <button onclick="window.print()" class="print-btn">🖨 Tisknout / Uložit jako PDF</button>
  <span class="print-hint">V dialogu tisku zvolte „Uložit jako PDF"</span>
</div>
...
<div class="page">
  ...
  <div class="footer">
    ...
    <div class="footer-right">
      ...
      266 266 792 &nbsp;·&nbsp; Po–Pá, 8:00–16:00
    </div>
  </div>

  <div style="margin-top:20px;background:#fdecea;border-radius:8px;padding:14px 18px;display:flex;align-items:center;justify-content:space-between">
    <div>
      <div style="font-size:10px;font-weight:700;color:#cb1419;text-transform:uppercase;letter-spacing:.08em;margin-bottom:3px">Váš technik</div>
      <div style="font-size:14px;font-weight:700;color:#1a1a1a">${technik}</div>
      ${techniktel?`<div style="font-size:13px;color:#4a4a4a;margin-top:2px">${techniktel}</div>`:''}
    </div>
    <div style="border-top:1px solid #cb1419;width:180px;text-align:center;padding-top:6px;font-size:11px;color:#888">Podpis technika</div>
  </div>

  <div style="margin-top:20px;border:1.5px solid #e5e0e0;border-radius:8px;padding:16px 18px">
    <div style="font-size:10px;font-weight:700;color:#cb1419;text-transform:uppercase;letter-spacing:.08em;margin-bottom:12px">Naše hodnocení</div>
    <div style="display:flex;gap:16px">
      <div style="flex:1;background:#f7f4f4;border-radius:8px;padding:12px 14px;display:flex;align-items:center;gap:12px">
        <div style="font-size:26px;font-weight:700;color:#1a1a1a">5.0</div>
        <div>
          <div style="color:#f4b400;font-size:15px;letter-spacing:1px">★★★★★</div>
          <div style="font-size:11px;color:#666;margin-top:2px">300+ hodnocení</div>
          <div style="font-size:11px;font-weight:700;color:#4285F4;margin-top:1px">Google</div>
        </div>
      </div>
      <div style="flex:1;background:#f7f4f4;border-radius:8px;padding:12px 14px;display:flex;align-items:center;gap:12px">
        <div style="font-size:26px;font-weight:700;color:#1a1a1a">4.9</div>
        <div>
          <div style="color:#f4b400;font-size:15px;letter-spacing:1px">★★★★★</div>
          <div style="font-size:11px;color:#666;margin-top:2px">170+ hodnocení</div>
          <div style="font-size:11px;font-weight:700;color:#cc0000;margin-top:1px">Firmy.cz</div>
        </div>
      </div>
    </div>
    <div style="margin-top:12px;font-size:11px;color:#888;font-style:italic;text-align:center">„Kvalitní stínění od profesionálů — zaměříme do 48 hod."</div>
  </div>
...
document.getElementById('btn-obj').addEventListener('click',function(){
  alert('Formát objednávky se připravuje...');
});

// ===================== INIT =====================
document.querySelectorAll('#klient,#zakazka,#adresa,#datum,#technik,#technik-tel').forEach(function(el){el.addEventListener('input',saveState);});
